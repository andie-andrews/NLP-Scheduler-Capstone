...
public sealed class OpenAiAssistantOrchestrator
{
    ...
    SchedulerApiClient schedulerApiClient)
  {
    ...
    {
      _isConfigured = false;
      _configurationError = "OPENAI_API_KEY is not configured. Set it in environment variables or user secrets.";
      return;
    }
  ...
  public async Task<OrchestratorResult> RunAsync(string userMessage, AuthUser user, JsonObject session, CancellationToken ct)
  {
    if (!_isConfigured)
    {
      return new OrchestratorResult
      {
        Response = new JsonObject
        {
          ...
          ["message"] = _configurationError ?? "OpenAI orchestration is not configured.",
        },
        Session = session,
      };
    }
    ...
    var assistantMessage = SelectAssistantMessage(first, fallbackText: "I’m sorry — I couldn’t generate a response just now.");
    ...
    if (toolCalls is not null && toolCalls.Count > 0)
    {
      ...
      var finalMessage = SelectAssistantMessage(followUp, fallbackText: "I completed the operation, but I could not summarize the result.");
  ...
  /// <summary>
  /// Returns the highest-ranked assistant message from completion choices, or a safe fallback message.
  /// </summary>
  private static JsonObject SelectAssistantMessage(JsonObject completion, string fallbackText)
  {
    ...
    if (choices is not null)
    {
      ...
      {
        if (choice is JsonObject choiceObject && choiceObject["message"] is JsonObject messageObject)
        {
