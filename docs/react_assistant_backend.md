...
If `OPENAI_API_KEY` is missing, the chat endpoint now returns a structured `configuration_error` response instead of throwing an unhandled startup/DI exception.
