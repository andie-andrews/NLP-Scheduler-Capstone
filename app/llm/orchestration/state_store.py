...
def get_pending_update_shift_state(session):
    ...
    if memory is None:
        return None
    return getattr(memory, PENDING_UPDATE_SHIFT_KEY, None)
...
def set_pending_update_shift_state(session, state):
    ...
    if memory is None:
        return
    setattr(memory, PENDING_UPDATE_SHIFT_KEY, state)
...
def get_pending_schedule_member_change_state(session):
    ...
    if memory is None:
        return None
    return getattr(memory, PENDING_SCHEDULE_MEMBER_CHANGE_KEY, None)
...
def set_pending_schedule_member_change_state(session, state):
    ...
    if memory is None:
        return
    setattr(memory, PENDING_SCHEDULE_MEMBER_CHANGE_KEY, state)
...
def get_pending_create_schedule_state(session):
    ...
    if memory is None:
        return None
    return getattr(memory, PENDING_CREATE_SCHEDULE_KEY, None)
...
def set_pending_create_schedule_state(session, state):
    ...
    if memory is None:
        return
    setattr(memory, PENDING_CREATE_SCHEDULE_KEY, state)
...
def get_pending_employee_operation_state(session):
    ...
    if memory is None:
        return None
    return getattr(memory, PENDING_EMPLOYEE_OPERATION_KEY, None)
...
def set_pending_employee_operation_state(session, state):
    ...
    if memory is None:
        return
    setattr(memory, PENDING_EMPLOYEE_OPERATION_KEY, state)
